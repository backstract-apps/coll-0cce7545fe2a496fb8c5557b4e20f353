...
async def put_appointment_types_id(
    db: Session, id: int, name: str, duration: int, description: str
):
...
async def post_appointments(db: Session, raw_data: schemas.PostAppointments):
    ...
    datetime: str = raw_data.datetime
...
async def put_appointments_id(
    db: Session, id: int, user_id: int, appointment_type_id: int, datetime: str
):
