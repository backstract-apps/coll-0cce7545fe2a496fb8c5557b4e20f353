...
async def put_appointment_types_id(id: int, name: Annotated[str, Query(max_length=100)], duration: int, description: Annotated[str, Query(max_length=100)], db: Session = Depends(get_db)):
    try:
        return await service.put_appointment_types_id(db, id, name, duration, description)
    except Exception as e:
        raise HTTPException(500, str(e))
...
async def put_appointments_id(id: int, user_id: int, appointment_type_id: int, datetime: Annotated[str, Query(max_length=100)], db: Session = Depends(get_db)):
    try:
        return await service.put_appointments_id(db, id, user_id, appointment_type_id, datetime)
    except Exception as e:
        raise HTTPException(500, str(e))
